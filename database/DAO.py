...
class DAO():
    ...
    def getAllNodes():
        ...
        for row in cursor:
            res.append(ArtObject(**row)) #associa tutti i campi

        cursor.close()
        conn.close()
        return res

    @staticmethod #poco usato
    def getEdgePeso(v1,v2):
        ...
        for row in cursor:
            ...
            res.append(row["peso"]) #associa tutti i campi
    ...
    def getAllEdges(idMapAO):
            ...
            for row in cursor:
                res.append(Arco(idMapAO[row["o1"]],idMapAO[row["o2"]],row["peso"]))  # associa tutti i campi
